...
% Parametres divers :
N = 150;					% Nombre de disques d'une configuration
...
for q = 1:q_max
    ...
    abscNouv = abscisse_nouveau * ones(size(abscisses_disques));
    ordNouv = ordonnee_nouveau * ones(size(ordonnees_disques));
    d = sqrt( ((abscisses_disques-abscNouv).^2) + ((ordonnees_disques-ordNouv).^2));
	if nvg_moyen_nouveau>nvg_moyen_courant && all(d > sqrt(2) * R)
